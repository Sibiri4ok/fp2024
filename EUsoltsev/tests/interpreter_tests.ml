...
let%expect_test "test_base_operation2" =
  test_interpret "let truth = false || false || true || false || false || false;; let result = print_bool truth";
  [%expect {|true|}]
;;
...
let%expect_test "test_base_operation4" =
  test_interpret "let create_adder x =
                  let adder y = x + y in
                  adder;;
...
let%expect_test "test_not_print" =
  test_interpret "let create_adder x =
                  let adder y = x + y in
                  adder;;
                  let fac n = if n < 2 then 1 else n * fac(n-1);; let x = 1;; let y = true";
  [%expect {||}]
;;

let%expect_test "test_factorial" =
  test_interpret "let rec fac n = if n < 2 then 1 else n * fac(n-1) ;; let result = print_int(fac 5)";
  [%expect {|120|}]
;;

let%expect_test "test_factorial_cps" =
  test_interpret "let rec fac_cps n k =
...
let%expect_test "test_fibonacci" =
  test_interpret "let rec fibo n = if n < 2 then 1 else fibo(n-1) + fibo(n-2)
                  let result = print_int(fibo 5)";
  [%expect {|8|}]
;;

let%expect_test "test_fix" =
  test_interpret "let rec fix f x = f (fix f) x
...
let%expect_test "test_annotate_fac" =
  test_interpret "let rec fac (n : int) (acc : int) = if n < 2 then acc else fac (n-1) (acc * n)
